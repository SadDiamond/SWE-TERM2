...
public class HangarGenerator : MacroGenerator
{
    ...
    {
        // Reserve spots for micro-generation near structures or in clumps
        for (int x = 4; x < width - 4; x++)
        {
            for (int z = 4; z < length - 4; z++)
            {
                ...
                {
                    if (CountWallNeighbors(map, x, z) > 0)
                    {
    ...
    private void GenerateHangar(MacroRegion[,] map, int width, int length, System.Random rng)
    {
        ...
        int padding = 1;
        CarveRect(map, padding, padding, width - 1 - padding, length - 1 - padding, MacroRegion.Open);
    ...
    private void GenerateInteriorInfrastructure(MacroRegion[,] map, int width, int length, System.Random rng)
    {
        ...
        // 1. Catwalk Network (Now at y=3)
        int catwalkCount = rng.Next(2, 4);
        for (int i = 0; i < catwalkCount; i++)
        {
            if (rng.NextDouble() > 0.5)
            {
                int rz = rng.Next(zMin + 2, zMax - 2);
                CarveRect(map, xMin, rz, xMax, rz, MacroRegion.Mezzanine);
            }
            else
            {
                int rx = rng.Next(xMin + 2, xMax - 2);
                CarveRect(map, rx, zMin, rx, zMax, MacroRegion.Mezzanine);
            }
        }

        // 2. Structural Islands
        int islandCount = rng.Next(4, 8);
        for (int i = 0; i < islandCount; i++)
        {
            ...
            if (roll < 0.45) // Raised Platform
            {
                int size = rng.Next(2, 4);
                CarveRect(map, rx - size, rz - size, rx + size, rz + size, MacroRegion.Platform);
            }
            else if (roll < 0.7) // Solid Pillar
            {
                int size = rng.Next(1, 2);
                CarveRect(map, rx - size, rz - size, rx + size, rz + size, MacroRegion.Wall);
            }
        }

        // 3. Scattered Cover and Hazards
        for (int x = xMin; x <= xMax; x++)
        {
            for (int z = zMin; z <= zMax; z++)
            {
                ...
                double roll = rng.NextDouble();
                if (roll < 0.03) map[x, z] = MacroRegion.Pit;
                else if (roll < 0.05) map[x, z] = MacroRegion.HighCover;
                else if (roll < 0.08) map[x, z] = MacroRegion.LowCover;
                else if (roll < 0.10) map[x, z] = MacroRegion.Hazard;
            }
        }
    }
